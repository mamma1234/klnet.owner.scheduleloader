...
class parser():
    ...
    def parsing(self):
        ...
        try:
            ...
            # for sheet_index in range(0, 1):
                ...
                #     continue
                # print("sheet.merged_cells", sheet.merged_cells)
            ...
            for j in range(0, len(excel[i])):
                ...
                for k in range(0, len(excel[i][j])):
                    ...
                    try:
                        ...
                        if "VSL NAME" in str(excel[i][j][k]) and "VOY" in str(excel[i][j][k+1]):
                            print("start ================> ", i,":",j,":",k)
                            ...
                            data.extend(self.get_routes(excel, i,j,k))
                            col_skip = k+1
                        # elif "Vessel" in str(excel[i][j][k]):
                        #     print("start ================> ", i,":",j,":",k)
                        #     data.extend(self.get_routes(excel, i,j,k))              
                        elif "VESSLE" in str(excel[i][j][k]) and "VOY" in str(excel[i][j][k+1]):
                            print("start ================> ", i,":",j,":",k)
                            data.extend(self.get_routes(excel, i,j,k))
                            col_skip = k+1
    ...
    def get_routes(self, excel, i, j, k):
        ...
        for kk in range(k, len(excel[i][j])):
            ...
            if "VSL NAME" in str(excel[i][j][kk]) and vessel_index < 0:
                vessel_index = kk

            if "VESSEL" in str(excel[i][j][kk]) and vessel_index < 0:
                vessel_index = kk

            if "VESSLE" in str(excel[i][j][kk]) and vessel_index < 0:
                vessel_index = kk

            if "VOY" in str(excel[i][j][kk]) and voy_index < 0:
                voy_index = kk

            if "ETD" in str(excel[i][j][kk]):
                tmp_etd_row = j+1
                break
            ...
            if "" != str(excel[i][j][kk]) and None != str(excel[i][j][kk]) and "*" not in str(excel[i][j][kk]) \
                ...
                and "VESSLE" not in str(excel[i][j][kk]) and "VSL NAME" not in str(excel[i][j][kk]):
...
# ETD Row Exception
        if tmp_etd_row > j:
            for kk in range(k, len(excel[i][tmp_etd_row])): 

                print(i,":",tmp_etd_row,":",kk,":",excel[i][tmp_etd_row][kk])

                if "" != str(excel[i][tmp_etd_row][kk]) and None != str(excel[i][tmp_etd_row][kk]) and "*" not in str(excel[i][tmp_etd_row][kk]) \
                    and "VESSEL" not in str(excel[i][tmp_etd_row][kk]) and "VOY" not in str(excel[i][tmp_etd_row][kk]) \
                    and "VESSLE" not in str(excel[i][tmp_etd_row][kk]) and "VSL NAME" not in str(excel[i][tmp_etd_row][kk]):
                    # and "Vessel" not in str(excel[i][j][kk]):
                    ports[str(kk)] = excel[i][tmp_etd_row][kk]
                    if "\n" in ports[str(kk)]:
                        ports[str(kk)] = ports[str(kk)].replace("\n"," ")
                    if port_start_index == 0:
                        port_start_index = kk
                if port_start_index > 0 and ("" == str(excel[i][tmp_etd_row][kk]) or None == str(excel[i][tmp_etd_row][kk]) or "*" in str(excel[i][tmp_etd_row][kk])):
                    port_end_index = kk-1
                    print("end ================> ", i,":",tmp_etd_row,":",kk)
                    break

                if kk == len(excel[i][tmp_etd_row])-1:
                    port_end_index = kk
                    print("end ================> ", i,":",j,":",kk)
        ...
        for jj in range(row_start+1, len(excel[i])):
            ...
            if tmp_etd_row > j:
                if "" == str(excel[i][jj+1][vessel_index]) or None == str(excel[i][jj+1][vessel_index]) \
                    or "*" in str(excel[i][jj+1][vessel_index]) or "반입 CY:" in str(excel[i][jj+1][vessel_index]):
                    row_end = jj
                    break
            else:
                if "*" in str(excel[i][jj][vessel_index]) or "반입 CY:" in str(excel[i][jj][vessel_index]):
                    row_end = jj-1
                    break   
                # if "" == str(excel[i][jj][vessel_index]) or None == str(excel[i][jj][vessel_index]) \
                #     or "*" in str(excel[i][jj][vessel_index]) or "반입 CY:" in str(excel[i][jj][vessel_index]):
                #     row_end = jj-1
                #     break   
            ...
            #     # print(i,":",j,":",kk,":", "confirm:", str(excel[i][jj][kk]))
            #     # print(i,":",j,":",kk,":", "merge", self.is_merge_extend(i, jj, kk))


            #     if kk == vessel_index:
            #         if ("" == str(excel[i][jj][kk]) or None == str(excel[i][jj][kk]) or "*" in str(excel[i][jj][kk])) \
            #             and not self.is_merge_extend(i, jj, kk):
            #             row_end = jj-1
            #             outerbreak = True
            #             break                 
        ...
        try:
            ...
            if "" != str(excel[i][row_start-2][col_start]) and None != str(excel[i][row_start-2][col_start]):
                svc = excel[i][row_start-2][col_start]
            ...
            for jj in range(row_start+1, row_end+1):
                ...
                if "" == str(excel[i][jj][vessel_index]):
                    continue
    ...
    def is_merge_extend(self, i, row, col):
        # print(row,"," ,col)
        # print( i,"self._sheets[i].merged_cells:",self._sheets[i].merged_cells)
        # print("crange", crange)
